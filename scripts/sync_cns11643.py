...
class CNS11643Syncer:
    ...
    def __init__(self, config: SyncConfig | None = None):
        ...
        self.session.headers.update({"User-Agent": "CNS11643-OpenData-Syncer/1.0"})
    ...
    def _download_and_process(self, filename: str) -> dict | None:
        """下載並處理單一檔案"""
        ...
        try:
            ...
            response = self.session.get(
                url, timeout=self.config.download_timeout, stream=True, verify=False
            )
    ...
    def _update_metadata(self, metadata: dict, downloaded_files: dict) -> None:
        ...
        # 保留最近 10 筆歷史
        metadata["sync_history"] = ([history_entry] + metadata.get("sync_history", []))[
            :10
        ]
    ...
    def _parse_release_version(self) -> str:
        ...
        # 使用 utf-8-sig 處理 BOM
        with open(release_path, encoding="utf-8-sig") as f:
            for line in f:
